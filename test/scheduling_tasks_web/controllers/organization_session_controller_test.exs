defmodule SchedulingTasksWeb.OrganizationSessionControllerTest do
  ...
  describe "POST /organizations/log_in" do
    ...
    test "logs the organization in with remember me", %{conn: conn, organization: organization} do
      ...
      assert conn.resp_cookies["_scheduling_tasks_web_organization_remember_me"]
      assert redirected_to(conn) == "/"
    end

    test "logs the organization in with return to", %{conn: conn, organization: organization} do
      conn =
        conn
        |> init_test_session(organization_return_to: "/foo/bar")
        |> post(Routes.organization_session_path(conn, :create), %{
          "organization" => %{
            "email" => organization.email,
            "password" => valid_organization_password()
          }
