defmodule SchedulingTasksWeb.OrganizationSettingsControllerTest do
  ...
  describe "PUT /organizations/settings (change password form)" do
    test "updates the organization password and resets tokens", %{conn: conn, organization: organization} do
      new_password_conn =
        put(conn, Routes.organization_settings_path(conn, :update), %{
  ...
  describe "PUT /organizations/settings (change email form)" do
    @tag :capture_log
    test "updates the organization email", %{conn: conn, organization: organization} do
      ...
      assert Organizations.get_organization_by_email(organization.email)
    end
  ...
  describe "GET /organizations/settings/confirm_email/:token" do
    ...
    test "updates the organization email once", %{conn: conn, organization: organization, token: token, email: email} do
      ...
      refute Organizations.get_organization_by_email(organization.email)
      assert Organizations.get_organization_by_email(email)
