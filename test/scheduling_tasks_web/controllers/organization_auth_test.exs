defmodule SchedulingTasksWeb.OrganizationAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, SchedulingTasksWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_organization/3" do
    test "stores the organization token in the session", %{conn: conn, organization: organization} do
      ...
      assert Organizations.get_organization_by_session_token(token)
    end
    ...
    test "writes a cookie if remember_me is configured", %{conn: conn, organization: organization} do
      ...
      assert get_session(conn, :organization_token) == conn.cookies[@remember_me_cookie]
  ...
  describe "logout_organization/1" do
    test "erases session and cookies", %{conn: conn, organization: organization} do
      organization_token = Organizations.generate_organization_session_token(organization)

      conn =
        conn
        |> put_session(:organization_token, organization_token)
        |> put_req_cookie(@remember_me_cookie, organization_token)
        |> fetch_cookies()
      ...
      assert redirected_to(conn) == "/"
      refute Organizations.get_organization_by_session_token(organization_token)
    end

    test "broadcasts to the given live_socket_id", %{conn: conn} do
      live_socket_id = "organizations_sessions:abcdef-token"
      SchedulingTasksWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "fetch_current_organization/2" do
    ...
    test "authenticates organization from cookies", %{conn: conn, organization: organization} do
      ...
      organization_token = logged_in_conn.cookies[@remember_me_cookie]
